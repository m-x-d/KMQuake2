...
static void CameraOn(edict_t *ent)
{
	if (!ent->client || !ent->client->spycam)
		return;
	...
	edict_t *monster = g_edicts + 1;
	for (int i = maxclients->value + 1; i < globals.num_edicts; i++, monster++)
	{
...
found_one:
	// Knightmare added- check this pointer!
...
{
	if (!activator->client || activator->client->spycam) // No client || already using camera
		return;
	...
	// If currently in thirdperson, turn it off
	if (tpp->integer && activator->client->chasetoggle)
		Cmd_Chasecam_Toggle(activator);

#ifdef KMQUAKE2_ENGINE_MOD
	// Knightmare- check for client-side chasecam
	if (!deathmatch->integer && !coop->integer && cl_thirdperson->integer)
	{
