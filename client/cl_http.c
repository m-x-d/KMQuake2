...
static int CL_HTTP_Progress(void *clientp, double dltotal, double dlnow, double ultotal, double ulnow)
{
	...
	if (!abortDownloads)
	{
		...
		if (dltotal)
		{
			CL_Download_Calculate_KBps((int)dlnow - prevSize, (int)dltotal); // Knightmare- added KB/s counter
			prevSize = (int)dlnow;
...
static size_t CL_HTTP_Header(void *ptr, size_t size, size_t nmemb, void *stream)
{
	...
	const size_t len = min(bytes, sizeof(headerBuff) - 1);
...
// Properly escapes a path with HTTP %encoding.
// libcurl's function seems to treat '/' and such as illegal chars and encodes almost the entire URL...
...
// Return value of false will cause standard UDP downloading to be used instead.
qboolean CL_QueueHTTPDownload(const char *quakePath)
{
	...
	const size_t len = strlen(quakePath);
	if (cl_http_filelists->value && len > 4 && !Q_stricmp(quakePath + len - 4, ".bsp"))
	{
...
// See if we're still busy with some downloads.
// Called by precacher just before it loads the map since we could be downloading the map.
// If we're busy still, it'll wait and CL_FinishHTTPDownload will pick up from where it left.
qboolean CL_PendingHTTPDownloads(void)
{
	return (cls.downloadServer[0] && pendingCount + handleCount > 0);
}
...
// A pak file just downloaded, let's see if we can remove some stuff from the queue which is in the .pak.
...
// A download finished, find out what it was, whether there were any errors and if so, how severe.
// If none, rename file and other such stuff.
	...
	do
	{
		...
		switch (result)
		{
			...
			case CURLE_OK:
			{
				...
				if (responseCode == 404)
				{
					const char *ext = COM_FileExtension(dl->queueEntry->quakePath);
					if (!Q_stricmp(ext, "pak") || !Q_stricmp(ext, "pk3"))
		...
		//FIXME:
		// Technically I shouldn't need to do this as curl will auto reuse the existing handle when you change the URL.
		// However, the handleCount goes all weird when reusing a download slot in this way.
...
// This calls curl_multi_perform do actually do stuff.
// Called every frame while connecting to minimise latency.
// Also starts new downloads if we're not doing the maximum already.
void CL_RunHTTPDownloads(void)
{
	...
	do
	{
		...
		{
			// Something either finished or errored out.
