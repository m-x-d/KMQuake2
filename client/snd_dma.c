...
void S_AddLoopSounds (void)
{
	...
	for (int i = 0; i < cl.frame.num_entities; i++)
	{
		...
		if (ent->solid == 31) // special value for bmodels
		{
			...
			S_SpatializeOrigin(origin_v, 255.0, dist_mult, &left_total, &right_total); // was ent->origin
			...
			for (int j = i + 1; j < cl.frame.num_entities; j++)
			{
				...
				S_SpatializeOrigin(origin_v, 255.0, dist_mult, &left, &right); // was ent->origin
		...
		{
			// find the total contribution of all sounds of this type
			S_SpatializeOrigin(ent->origin, 255.0, dist_mult, &left_total, &right_total);
			for (int j = i + 1; j < cl.frame.num_entities; j++)
			{
				if (sounds[j] != sounds[i])
					continue;
				...
				S_SpatializeOrigin(ent->origin, 255.0, dist_mult, &left, &right);
...
void S_RawSamples (int samples, int rate, int width, int channels, byte *data, qboolean music)
{
	...
	if (channels == 2 && width == 2)
	{
		if (scale == 1.0)
		{
			...
			for (int i = 0; i < samples; i++)
			{
				...
				s_rawend++;
				s_rawsamples[dst].left =  LittleShort(((short *)data)[i * 2]) * snd_vol; // << 8;
				s_rawsamples[dst].right = LittleShort(((short *)data)[i * 2 + 1]) * snd_vol; // << 8;
			}
		}
		else
		{
			for (int i = 0; ; i++)
			{
				...
				const int dst = s_rawend & (MAX_RAW_SAMPLES - 1);
				s_rawend++;
				s_rawsamples[dst].left =  LittleShort(((short *)data)[src * 2]) * snd_vol; // << 8;
				s_rawsamples[dst].right = LittleShort(((short *)data)[src * 2 + 1]) * snd_vol; // << 8;
			}
		}
	}
	else if (channels == 1 && width == 2)
	{
		for (int i = 0; ; i++)
		{
			...
			const int dst = s_rawend & (MAX_RAW_SAMPLES - 1);
			s_rawend++;
			s_rawsamples[dst].left =  LittleShort(((short *)data)[src]) * snd_vol; // << 8;
			s_rawsamples[dst].right = LittleShort(((short *)data)[src]) * snd_vol; // << 8;
		}
	}
	else if (channels == 2 && width == 1)
	{
		for (int i = 0; ; i++)
		{
			...
			const int dst = s_rawend & (MAX_RAW_SAMPLES - 1);
			s_rawend++;
			s_rawsamples[dst].left =  ( ((char *)data)[src * 2] << 8 ) * snd_vol; // << 16;
	...
	else if (channels == 1 && width == 1)
	{
		for (int i = 0; ; i++)
		{
			...
			const int dst = s_rawend & (MAX_RAW_SAMPLES - 1);
			s_rawend++;
			s_rawsamples[dst].left =  ( (((byte *)data)[src] - 128) << 8 ) * snd_vol; // << 16;
...
void S_Update (vec3_t origin, vec3_t forward, vec3_t right, vec3_t up)
{
	...
	// if the laoding plaque is up, clear everything out to make sure we aren't looping a dirty dma buffer while loading
