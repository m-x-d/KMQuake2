...
{
	return (chan->reliable_length == 0); // reliable_length > 0 -> waiting for ack
}
...
void Netchan_Transmit(netchan_t *chan, int length, byte *data)
{
	...
	const unsigned w1 = (chan->outgoing_sequence & ~(1 << 31)) | (send_reliable << 31);
	const unsigned w2 = (chan->incoming_sequence & ~(1 << 31)) | (chan->incoming_reliable_sequence << 31);
...
qboolean Netchan_Process(netchan_t *chan, sizebuf_t *msg)
{
	...
	const unsigned reliable_message = sequence >> 31;
	const unsigned reliable_ack = sequence_ack >> 31;
